...
% files
%
:- multifile at/3, parked/2, delivered/2,dirty/2, holding/2, storedAt/3.
...
poss( grab(Car) ) :-
  at(Agent,Where,S),
  storedAt(Car,Where,S),
  agent(Agent),
  car(Car).

poss( store(Car,Where) ) :-
  at(Agent,Where,S),
  at(Car,Where,S),
  parked(Car,S),
  holding(Car,S),
...
holding( Car,result(A,S) ) :-
    A = grab(Car);
    holding(Car,S), not(A = store(Car,_)), not(A = deliver(Car)).
storedAt( Car,Where,result(A,S) ) :-
    A = store(Car,Where);
    storedAt(Car,Where,S), not(A = grab(Car)).
