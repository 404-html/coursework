...
% files
%
:- multifile nextTo/2, at/3, parked/2, delivered/2, car/1, agent/1, dirty/2, space/2.
...
space( Number,result(A,S) ) :-
    (space(Prev,S), not(A=drive(Car,_)),parked(Car,S), not(A=park(_)), Number = Prev);
    (space(Prev,S), A = park(_), Number = Prev - 1);
    (space(Prev,S), A = drive(Car,_),parked(Car,S), Number = Prev + 1).
