...
def N_phrase_num(tr):
    ...
    if (tr.label() == 'N'):
        return tr[0][1]  # the s or p from Ns or Np (N[s] -> "N's'" etc.)
    elif (tr.label() == 'Nom'):
        ...
        if (len(tr) == 2):
            if (tr[0].label() == 'P'):
                return '' #I'm not sure what to do with P
            else:
                return N_phrase_num(tr[1])
        else:
            return N_phrase_num(tr[0])
...
def check_node(tr):
    ...
    if (rule == 'S -> WHICH Nom QP QM'):
        ...
        return ((matches (N_phrase_num(tr[0]), V_phrase_num(tr[1]))) and (V_phrase_num(tr[2]) == 'p'))
    elif (rule == 'VP -> BE NP'):
        return (matches (N_phrase_num(tr[0]), V_phrase_num(tr[1])))
    elif (rule == 'VP -> VP AND VP'):
        return (matches (N_phrase_num(tr[0]), V_phrase_num(tr[2])))
    elif (rule == 'NP -> Nom'):
