...
class MH_TypecheckerCOPY {
	...
	throws TypeError, UnknownVariable {
    	//copied
    	MH_TYPE type = null;
		if (exp.isVAR()) type = env.typeOf(exp.value());
		else if (exp.isNUM()) type = IntegerType;
		else if (exp.isBOOLEAN()) type = BoolType;
		else if (exp.isAPP()) {
			MH_TYPE type1 = computeType(exp.first(), env); // type according to the function def
			MH_TYPE type2 = type1.right(); // what the first argument should be
			MH_TYPE type3 = computeType(exp.second(), env); // what the first argument is
			MH_TYPE type4 = type1.left(); // what is the type of the expression
			if (!type3.equals(type4))
				throw new TypeError("Function expected argument of type "
						+ type4.toString() + ", " + type3.toString()
						+ " was found instead.");
			type = type2;
		} else if (exp.isINFIX()) {
			MH_TYPE type1 = computeType(exp.first(), env);
			MH_TYPE type2 = computeType(exp.second(), env);
			String infixop = exp.infixOp();
			if ((!type1.equals(IntegerType)) || (!type2.equals(IntegerType)))
				throw new TypeError("Incompatible types in " + infixop
						+ " infix operation: " + type1.toString() + " and "
						+ type2.toString() + ".");
			if (infixop == "+" || infixop == "-") {
				type = IntegerType;
			} else if (infixop == "<=" || infixop == "==") {
				type = BoolType;
			} else
				throw new TypeError("Unsupported operator: " + infixop + ".");
		} else if (exp.isIF()) {
			MH_TYPE type1 = computeType(exp.first(), env);
			if (!type1.equals(BoolType))
				throw new TypeError("Condition expression in an 'if' statement should be a boolean type.");
			MH_TYPE type2 = computeType(exp.second(), env);
			MH_TYPE type3 = computeType(exp.third(), env);
			if (!type2.equals(type3))
				throw new TypeError("Different types in 'then' and 'else' parts of a conditional statement");
			type = type2;
		}
		return type;
		//copied
