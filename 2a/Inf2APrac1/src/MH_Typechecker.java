...
class MH_Typechecker {
	...
	throws TypeError, UnknownVariable {
    	...
    	if (exp.isAPP()){
    		MH_TYPE function = computeType(exp.first(), env);
    		MH_TYPE expect = function.left();
    		MH_TYPE outcome = function.right();
    		MH_TYPE actual = computeType(exp.second(), env);
    		if (expect.equals(actual)){
        		ret = outcome;
    		}else{
    			throw new TypeError("Function type mismatch.\nExpected type: "
    					+ expect.toString() + "\nActual type: " + actual.toString());
    		}
    	}
    	if (exp.isINFIX()){
			MH_TYPE left = computeType(exp.first(), env);
			MH_TYPE right = computeType(exp.second(), env);
			String op = exp.infixOp();
			if (!(left.equals(IntegerType)&&right.equals(IntegerType))){
				throw new TypeError("Infix operation \"" + op + "\" type mismatch.\n"
			                       +"Expected type: Integer and Integer\n"
    				               +"Actual type: " + left.toString() +" and " + right.toString());
			}
			if (op == "+" || op == "-") {
				ret = IntegerType;
			} else {
				if (op == "<=" || op == "==") {
					...
					throw new TypeError("Infix operation \"" + op + "\" unknown.\n"
		                       +"Expected operations: \"+\" \"-\" \"<=\" \"==\"\n");
				}	
			}
    	}
    	if (exp.isIF()){
    		MH_TYPE ifexp = computeType(exp.first(), env);
    		MH_TYPE thenexp = computeType(exp.second(), env);
    		MH_TYPE elseexp = computeType(exp.third(), env);
    		if (!ifexp.equals(BoolType)){
    			throw new TypeError("Expression after \"if\" should be Boolean type.");
    		}
    		if (!thenexp.equals(elseexp)){
				throw new TypeError("Expressions after \"then\" and \"else\" are different types.");
    		}
    		ret = thenexp;
    	}
    	return ret;
