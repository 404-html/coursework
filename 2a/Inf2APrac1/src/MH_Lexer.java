...
static class VarAcceptor extends GenAcceptor implements DFA {
	...
	case 1: if (CharTypes.isSmall(c)) return 1 ; else 
	        if (CharTypes.isLarge(c)) return 1 ; else 
			if (CharTypes.isDigit(c)) return 1 ; else 
...
static class BooleanAcceptor extends GenAcceptor implements DFA {
    ...
    int nextState (int state, char c) {
	switch (state) {
	case 0: if (c=='T') return 1 ; else 
		    if (c=='F') return 5 ; else return 9 ;
...
static class CommentAcceptor extends GenAcceptor implements DFA {
	...
	case 2: if (c == '-')                return 2 ; else 
		    if (CharTypes.isSymbolic(c)) return 4 ; else 
			if (CharTypes.isNewline(c))  return 4 ; else return 3 ;
...
static class TokAcceptor extends GenAcceptor implements DFA {
    ...
    int nextState (int state, char c) {
    	if (state == tokLen+1) return state; else
    	if (state == tokLen) return state+1; else
	...
	static DFA lParenAcc = new TokAcceptor("(") ;
	static DFA rParenAcc = new TokAcceptor(")") ;
	static DFA semicoAcc = new TokAcceptor(";") ;
		
	static DFA[] MH_acceptors = 
	new DFA[] {ifAcc, thenAcc, elseAcc, varAcc, intAcc, boolAcc, booleanAcc,
    comAcc, symAcc, numAcc, lParenAcc, rParenAcc, semicoAcc, whiteAcc} ;
    //The {if, then, else} should be before VAR to be read.
	//With ---should be comment, comAcc should be before symAcc
	//Since {(,),;} is not symbolic, it's okay to be put anywhere?
	//{Integer,Bool,boolean} starts with large, and is all different
