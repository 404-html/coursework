...
public:
	...
	void free_pages(PageDescriptor *pgd, int order) override
	{
		...
		for (int i = order;i<MAX_ORDER-1;i++){
			PageDescriptor *buddy = buddy_of(pgd,i);
			PageDescriptor **found = &_free_areas[i];
			bool merge = false;
			while (*found) {
				if (buddy == *found) {
					merge = true;
					...
					found = &(*found)->next_free;
				}
			}
			if (merge) {
				pgd = *merge_block(&pgd,i);
			} else {
				break;
			}
		}
	...
	bool reserve_page(PageDescriptor *pgd)
	{
		...
		PageDescriptor *location = NULL;
		int locationOrder = 0;
		for (int i = 0; i < MAX_ORDER; i++){
			PageDescriptor *found = _free_areas[i];
			while (found && found <= pgd){
				if (found >= location){
					location = found;
					locationOrder = i;
				}
		...
		while (locationOrder > 0){
			PageDescriptor *buddy1 = split_block(&location,locationOrder--);
			PageDescriptor *buddy2 = buddy_of(buddy1,locationOrder);
			if (pgd < buddy2) {
				location = buddy1;
			} else {
				location = buddy2;
	...
	bool init(PageDescriptor *page_descriptors, uint64_t nr_page_descriptors) override
	{
		...
		while (nr_page_descriptors > 0) {
		  nr_page_descriptors -= pages_per_block(MAX_ORDER - 1);
			page_descriptors->next_free = page_descriptors + pages_per_block(MAX_ORDER - 1);
			page_descriptors = page_descriptors->next_free;
		}
		page_descriptors->next_free = NULL;
